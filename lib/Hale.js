...
Hale.prototype.publicCheckRequest = function (request, response) {
  var hapi = this.plugin.hapi;

  performCheck(this)
    .then(function serveResult(data) {
      if (data.status === 'FAIL') {
        response(hapi.error.internal('Healthcheck failed'));
      }
      else {
        response();
      }
    }).catch(function failure(err) {
      response(hapi.error.internal('Healthcheck failed', err));
    });
};
...
function performCheck(hale) {
  var maxLevel = 0;

  var results = _.map(hale.checks, function runTest(check) {
  ...
  var metadata = hale.metadata;
  return Promise.all(results).then(function buildResponse(checks) {
    var data = _.defaults({
      time: Date.now(),
...
StatusCollector.prototype.mark = function(name) {
  ...
  collector.times.push(data);

  return function markDone(label) {
    data.marks.push({
